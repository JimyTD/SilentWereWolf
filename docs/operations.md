...
最近核查时间：2026-09-18（提交 `0e801fa` 更新发布，含 AI 阶梯模型链 v0.3.0 与 TokenHub 密钥接入）。
...
- 当前运行版本：提交 `0e801fa`（v0.3.0，容器内 `package.json` 已核对），发布目录 `/root/SilentWereWolf_0e801fa`。
- 密钥文件 `/root/silentwerewolf-secrets/silentwerewolf.env` 存在，目录 `700`、文件 `600`，容器内已注入 `ZHIPU_API_KEY` 与 `TOKENHUB_API_KEY`。
- 密钥写入方式：由 Agent 通过 Lighthouse 命令写入（**正常路径**），唯一硬约束是不得进入 Git 仓库（见 §4.2）。
- 可回滚版本：`/root/SilentWereWolf_1c6b0ea`、`/root/SilentWereWolf_4685d2b`、`/root/SilentWereWolf_38814a1`（v0.2.2，`git clone` 目录）、`/root/SilentWereWolf_20260903100000`、`/root/SilentWereWolf_20260831142146`、`/root/SilentWereWolf_20260827145231`。
...
- 更新后验证结论：`http://127.0.0.1:8081/` 与 `http://106.55.228.236:8081/` 均返回 200；启动日志打印完整 **14 档且无 `(skip: no key)`**；`/v1/models` 用真实密钥拉通（**108 个可调用模型，其中 8 个 pre-offline**），且**无任何链上档被剔除**；QQBot 容器未受影响。
